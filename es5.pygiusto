...
def main():
    ...
    while solutions < 10:
        ...
        if soluzione_ok(scacchiera) and scacchiera not in lista_soluzioni: 
            # se la soluzione è buona, scrive
            print(f'Found solution {scacchiera} in {time.time() - start_time} s.')
            
            lista_soluzioni.append (scacchiera)   #.append aggiunge qualcosa alla lista: in questo caso carico nella lista la scacchiera 

            tempo = time.time() - start_time + tempo      #sommo i diversi tempi (al secondo sommo il tempo del primo etc)
            ...
            start_time = time.time()
    media_tempo = tempo / 10                   #faccio divisione normale non per numeir interi 
    print (f'il tempo medio in cui si trova 1 soluzione {media_tempo} s.')
